...
Fixed in commit acabf0d. `search` and `get_neighbors` now return metadata-only by default. Added `include_content` parameter (default: false) to opt-in to full content.
