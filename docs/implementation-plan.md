...
- Centrality caching strategy (PageRank recomputation frequency)
...
- **File Watching Details**: What debounce interval? (100ms reasonable default) How to handle partial reads during write? Recursive depth? `.roux/` ignored automatically?
- **Centrality Computation Timing**: Compute PageRank on init, invalidate on change, recompute lazily on query? Or background job?
...
- **ID Derivation Edge Cases**: What if title contains special characters? Slashes? Unicode? Document normalization rules.
